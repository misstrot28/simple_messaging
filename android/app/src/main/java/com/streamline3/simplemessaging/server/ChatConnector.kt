...
open class ChatConnector(client: Socket) : Runnable, Observer {
    ...
    private fun readUserInput(): ChatMessage {
        ...
        return Gson().fromJson(input, ChatMessage::class.java)
    }
    ...
    override fun run() {
        ...
        while (true) {
            try {
                ...
                if (Users.checkUsername(newMessage.message.toString()) && newMessage.message?.trim()?.isNotEmpty() == true) {
        ...
        while (running) {
            try {
                ...
                if (newMessage.message?.isNotBlank() == true) {
                        ...
                        when (interpretedInput) {
                            ...
                            "CHANGE USERNAME" -> {
                                ...
                                if (Users.changeUsername(
                                        userName,
                                        newUsernameInput.message.toString()
                                    )
                                ) {
                                    userName = newUsernameInput.message.toString()
